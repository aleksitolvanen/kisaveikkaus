...
const state = { players: loadSel(), view: 'standings', filterOpen: false, dayFilter: 'all',
  predMode: 'lohko', cmpA: null, cmpB: null, odds: null };
...
function predSeg(){
  ...
  [['lohko','Lohko'],['cup','Cup'],['muut','Muut']].forEach(function(o){
    ...
    b.onclick=function(){ state.predMode=o[0]; renderPredictions(); };
    seg.appendChild(b);
  });
  return seg;
}
// Cup-vaiheen veikkaukset: rivit = veikatut joukkueet kierroksittain, ● = veikattu jatkoon.
function renderCupInto(box, players){
  ...
  T.cupRounds.forEach(function(rd){
    ...
    var pop={};
    NAMES.forEach(function(n){
      var v=P[n].cup&&P[n].cup[rd.key];
      var arr = rd.key==='champion' ? (v?[v]:[]) : (v||[]);
      arr.forEach(function(t){ pop[t]=(pop[t]||0)+1; });
    });
    var teams=Object.keys(pop).sort(function(a,b){ return pop[b]-pop[a] || a.localeCompare(b); });
    ...
    if(!teams.length){ rows.push({ label:'–', actual:null, cellFor:function(){ return {txt:''}; } }); return; }
    teams.forEach(function(t){
      var reached = actual.indexOf(t)>=0;
      rows.push({ label:t, mono:true, title:teamName(t),
        actual: reached?'✓':(resolved?'✗':'–'), actualTitle:teamName(t),
        cellFor:(function(team,key,reach,res){ return function(n){
          var v=P[n].cup&&P[n].cup[key];
          var picked = key==='champion' ? (v===team) : (Array.isArray(v)&&v.indexOf(team)>=0);
          return picked ? { txt:'●', title:teamName(team), cls: res?(reach?'c3':'c0'):'' } : { txt:'' };
        }; })(t, rd.key, reached, resolved) });
    });
  });
  var w=buildMatrix(players, null, rows); w.style.flex='1 1 auto'; w.style.minHeight='0';
  box.appendChild(w); freezeOffsets(w);
  box.appendChild(el('div','hint','● = veikattu jatkoon. Vihreä = oikein, harmaa = väärin (kun kierros ratkennut). Tulos-sarake: ✓ = joukkue pääsi, ✗ = ei.'));
}
...
// Muut: sikajengi + maalintekijä (oma välilehti)
function renderMuutInto(box, players){
  ...
  var w=buildMatrix(players, null, muut); box.appendChild(w); freezeOffsets(w);
}
