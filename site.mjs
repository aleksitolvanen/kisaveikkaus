...
.bteam .tn{display:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.bbz{margin-top:16px}
/* kännykällä: kierros kerrallaan (snap-pyyhkäisy), koko maannimet — kuten FIFA:n mobiilikaavio */
@media(max-width:560px){
  .bwrap{scroll-snap-type:x mandatory}
  .bracket{min-width:0}
  .bcol{flex:0 0 76vw;scroll-snap-align:center}
  .blab{font-size:11px;height:18px}
  .bcell{font-size:13px;padding:5px 9px}
  .bteam{line-height:1.8}
  .bteam.named .tn{display:inline}
  .bteam.named .tc{display:none}
}
...
function matchDetail(m){
  ...
  // Koko maiden nimet (kännykällä rivin tooltip ei ole käytettävissä).
  var full=pairTitle(m.home,m.away);
  if(full!==m.home+' – '+m.away) d.appendChild(el('div','mnames',full));
...
function renderBracket(){
  ...
  function cell(num, parentNum, extraCls){
    ...
    [[m.home,sc[0]],[m.away,sc[1]]].forEach(function(t,j){
      ...
      var nm = m.real ? teamName(code) : null;
      var row=el('div','bteam'+(m.real?'':' tbd')+(adv?' win':'')+(nm&&nm!==code?' named':''));
      row.appendChild(el('span','tc',code));
      if(nm&&nm!==code) row.appendChild(el('span','tn',nm));
