...
.lightbox img.rot{max-width:100vh;max-height:100vw}
...
// Kuvan koko ruudun katselu: nipistyszoomi + raahaus + tuplanapautuszoomi
// kohdistuvat kuvaan (ei sivuun — touch-action:none estää sivu-zoomin joka
// jättäisi sivun jumiin). ⟳ kääntää 90 astetta, tausta/×/ESC sulkee.
function openLightbox(src, alt){
  ...
  var scale=1, tx=0, ty=0, rotated=false;
  function apply(){
    img.style.transform='translate('+tx.toFixed(1)+'px,'+ty.toFixed(1)+'px) scale('+scale.toFixed(3)+')'+(rotated?' rotate(90deg)':'');
  }
  function reset(){ scale=1; tx=0; ty=0; apply(); }

  // Pointer-eleet: 1 sormi = raahaus, 2 sormea = nipistyszoomi
  var pts={}, lastDist=0, moved=0, tapAt=0;
  function pcount(){ return Object.keys(pts).length; }
  function mid(){ var k=Object.keys(pts); return { x:(pts[k[0]].x+pts[k[1]].x)/2, y:(pts[k[0]].y+pts[k[1]].y)/2 }; }
  function dist(){ var k=Object.keys(pts); var dx=pts[k[0]].x-pts[k[1]].x, dy=pts[k[0]].y-pts[k[1]].y; return Math.sqrt(dx*dx+dy*dy); }
  lb.addEventListener('pointerdown', function(e){
    if(e.target===rot||e.target===cls) return;
    pts[e.pointerId]={x:e.clientX,y:e.clientY};
    lb.setPointerCapture(e.pointerId);
    if(pcount()===2) lastDist=dist();
    moved=0;
    e.preventDefault();
  });
  lb.addEventListener('pointermove', function(e){
    ...
    if(pcount()===1){
      var dx=e.clientX-pts[e.pointerId].x, dy=e.clientY-pts[e.pointerId].y;
      ...
      pts[e.pointerId]={x:e.clientX,y:e.clientY};
      var d=dist();
      if(lastDist>0){ scale=Math.min(5, Math.max(1, scale*d/lastDist)); if(scale===1){ tx=0; ty=0; } apply(); }
      lastDist=d; moved=99;
    }
  });
  function lift(e){
    ...
    if(moved<8){
      var now=Date.now();
      if(now-tapAt<320){ // tuplanapautus: zoomaa sisään/ulos
        if(scale>1) reset(); else { scale=2.5; apply(); }
        tapAt=0;
      } else {
        tapAt=now;
        setTimeout(function(){ // yksi napautus zoomaamattomana sulkee
          if(tapAt&&Date.now()-tapAt>=300&&scale===1&&pcount()===0) close();
        }, 320);
      }
    }
  }
  ...
  function close(){ tapAt=0; lb.remove(); document.removeEventListener('keydown', onKey); }
  function onKey(e){ if(e.key==='Escape') close(); }
  rot.onclick=function(e){ e.stopPropagation(); rotated=!rotated; img.classList.toggle('rot'); reset(); };
